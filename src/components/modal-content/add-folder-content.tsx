...
export function AddFolderContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result.error) {
      ...
      closeModal()
      if (modal?.type === "add-folder" && modal.onSuccess) {
        modal.onSuccess()
      }
    }
  }
  ...
  if (modal?.type !== "add-folder") return null
      ...
      <div className="border-border/40 mb-6 flex items-center justify-between border-b pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">Create a folder to organize bookmarks</p>
          </div>
        </div>
        <button
